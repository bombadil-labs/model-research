"""Stage 4b: relation lens.  For a held-out domain d and role pair S->T:
...
ap.add_argument("--lam", default="1,2"); ap.add_argument("--ridge", type=float, default=10.0); ap.add_argument("--domains", default=None); ap.add_argument("--out", default=None)
a = ap.parse_args()
...
for d in test_doms:
    ...
    for S, T in pairs:
        ...
        wrong_src = avg[d][(Si + 3) % R] - mu[(Si + 3) % R]; pred_wrong = op(wrong_src); pred_wrong *= pn / (np.linalg.norm(pred_wrong) + 1e-9)
        ...
        for lam in lams:
            ...
            for cname, extra in conds.items():
                ...
                res.append(dict(domain=d, src=S, dst=T, lam=lam, cond="rand" if cname.startswith("rand") else cname,
